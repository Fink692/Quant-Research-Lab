...
class BacktestEngine:
    ...
    def _clip_weights(self, weights: pd.Series) -> pd.Series:
        ...
        if gross > self.max_gross_exposure and gross > 0:
            clean = clean * (self.max_gross_exposure / gross)
        return clean
    ...
    def run(
        ...
        for date, prices_row in self.prices.iterrows():
            ...
            if not target_row.empty:
                aligned_target = self._clip_weights(target_row.reindex(prices_row.index).fillna(0.0))
        ...
        returns_series = pd.Series(returns, index=index, name=name).replace([np.inf, -np.inf], np.nan).fillna(0.0)
